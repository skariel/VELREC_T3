...
Logging.configure(filename= "realization_$(LOGGING_FN)/log.txt")
...
elseif OPT_TYPE == "fwd" && SIM_TYPE == "dyn"
    optimize_dyn_vs_pushed_pos!(rho, opos_i, pos, m, 0.01, 1.0, 0.21, 200);
...
elseif OPT_TYPE == "laminar" && SIM_TYPE == "dyn"
    back_optimize_zeld_vs_pushed_pos!(vx,vy,vz, rho, opos_i, pos, m, 0.01, 1.0, 50, 0.21, 200);
else
